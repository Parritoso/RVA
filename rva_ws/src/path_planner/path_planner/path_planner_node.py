...
class PlannerNode(Node):
    def __init__(self):
        ...
        self.goal = None
        self.goal_pose = None
        self.path_published_for_goal = None
        self.waiting_for_costmap_logged = False
        ...
        self.retry_timer = self.create_timer(0.5, self.try_compute_path)
    ...
    def try_compute_path(self):
        ...
        if self.planner is None:
            if not self.waiting_for_costmap_logged:
                self.get_logger().warn("Goal stored, waiting for costmap before planning")
                self.waiting_for_costmap_logged = True
            return

        goal_signature = (
            round(self.goal_pose.pose.position.x, 3),
            round(self.goal_pose.pose.position.y, 3),
        )
        if self.path_published_for_goal == goal_signature:
            return
        ...
        try:
            ...
            if self.tf_buffer.can_transform(
                ...
                if not self.waiting_for_tf_logged:
                    self.get_logger().warn("Goal stored, waiting for robot transform before planning")
        ...
        if initial_pos is not None:
            ...
            if self.compute_path(ix, iy, gx, gy):
                self.path_published_for_goal = goal_signature
    ...
    def compute_path(self, ix, iy, gx, gy):
        if self.planner is not None:
            self.get_logger().info("self.planner not None")
            ...
            if path is not None:
                self.get_logger().info("path not None")
                xpath = path[0]
                ypath = path[1]
